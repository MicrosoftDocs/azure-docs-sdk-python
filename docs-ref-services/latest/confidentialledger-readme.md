...
Clients can leverage the receipt verification library in the SDK to verify write transaction receipts issued by Azure Confidential Ledger instances. The utility can be used to fully verify receipts offline as the verification algorithm does not require to be connected to a Confidential ledger or any other Azure service.
